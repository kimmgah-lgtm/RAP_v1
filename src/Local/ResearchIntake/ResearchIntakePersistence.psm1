...
function Get-RapResearchIntakeBody($Store){
    [ordered]@{SchemaVersion=3;KnownProjectIds=@($Store.KnownProjectIds);Questions=ConvertTo-RapIntakeMapObject $Store.Questions;Searches=ConvertTo-RapIntakeMapObject $Store.Searches;Inbox=ConvertTo-RapIntakeMapObject $Store.Inbox;Promotions=ConvertTo-RapIntakeMapObject $Store.Promotions;IdentityResults=ConvertTo-RapIntakeMapObject $Store.IdentityResults;LookupSnapshot=$Store.LookupSnapshot;LookupResults=ConvertTo-RapIntakeMapObject $Store.LookupResults;Decisions=ConvertTo-RapIntakeMapObject $Store.Decisions;Lifecycles=ConvertTo-RapIntakeMapObject $Store.Lifecycles;CanonicalPapers=@($Store.CanonicalPapers);ZoteroItems=@($Store.ZoteroItems);Audit=@($Store.Audit);Counters=$Store.Counters;ProductionWrite=[string]$Store.ProductionWrite;ProductionPilot=[string]$Store.ProductionPilot}
}
...
function Save-RapResearchIntakeStore {
    ...
    $path=Initialize-RapResearchIntakeStore $Store.DatabasePath;$body=Get-RapResearchIntakeBody $Store;$json=$body|ConvertTo-Json -Depth 80 -Compress;$hash=Get-RapIntakePersistenceHash $json;$payload=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($json));$now=[DateTimeOffset]::UtcNow.ToString('o');$sql="BEGIN IMMEDIATE;INSERT INTO ResearchIntakeStore(StoreId,Payload,RecordHash,SchemaVersion,UpdatedUtc) VALUES('PRIMARY',$(ConvertTo-RapIntakeSql $payload),'$hash',3,$(ConvertTo-RapIntakeSql $now)) ON CONFLICT(StoreId) DO UPDATE SET Payload=excluded.Payload,RecordHash=excluded.RecordHash,SchemaVersion=excluded.SchemaVersion,UpdatedUtc=excluded.UpdatedUtc;COMMIT;";[Rap.NativeSqlite]::Execute($path,$sql,5000);$Store
}
function Assert-RapIntakePersistedBody($Body){
    if($null-eq$Body-or[long]$Body.SchemaVersion-ne3){throw 'RESEARCH_INTAKE_SCHEMA_UNSUPPORTED'}
    ...
    foreach($p in $Body.Decisions.PSObject.Properties){$x=$p.Value;if($p.Name-cne$x.CandidateId){throw 'RESEARCH_INTAKE_PERSISTENCE_CORRUPT'};foreach($n in @('BindingVersion','DecisionId','ProjectId','ResearchQuestionId','SearchExecutionId','PromotionHash','CandidateIdentityHash','IdentityEvidenceHash','LookupExecutionId','LookupEvidenceHash','DecisionBindingHash','RevalidationSequence','RevalidationEvidenceHash','DecisionAuditSequence','DecidedAt')){if(-not$x.PSObject.Properties[$n]-or[string]::IsNullOrWhiteSpace([string]$x.$n)){throw 'RESEARCH_INTAKE_PERSISTENCE_CORRUPT'}}}
