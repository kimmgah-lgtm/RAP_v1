...
function Get-RapResearchIntakeBody($Store){
    [ordered]@{SchemaVersion=1;KnownProjectIds=@($Store.KnownProjectIds);Questions=ConvertTo-RapIntakeMapObject $Store.Questions;Searches=ConvertTo-RapIntakeMapObject $Store.Searches;Inbox=ConvertTo-RapIntakeMapObject $Store.Inbox;Promotions=ConvertTo-RapIntakeMapObject $Store.Promotions;IdentityResults=ConvertTo-RapIntakeMapObject $Store.IdentityResults;LookupResults=ConvertTo-RapIntakeMapObject $Store.LookupResults;Decisions=ConvertTo-RapIntakeMapObject $Store.Decisions;Lifecycles=ConvertTo-RapIntakeMapObject $Store.Lifecycles;CanonicalPapers=@($Store.CanonicalPapers);ZoteroItems=@($Store.ZoteroItems);Audit=@($Store.Audit);Counters=$Store.Counters;ProductionWrite=[string]$Store.ProductionWrite;ProductionPilot=[string]$Store.ProductionPilot}
}
...
function Save-RapResearchIntakeStore {
    ...
    $path=Initialize-RapResearchIntakeStore $Store.DatabasePath;$body=Get-RapResearchIntakeBody $Store;$json=$body|ConvertTo-Json -Depth 80 -Compress;$hash=Get-RapIntakePersistenceHash $json;$payload=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($json));$now=[DateTimeOffset]::UtcNow.ToString('o');$sql="BEGIN IMMEDIATE;INSERT INTO ResearchIntakeStore(StoreId,Payload,RecordHash,SchemaVersion,UpdatedUtc) VALUES('PRIMARY',$(ConvertTo-RapIntakeSql $payload),'$hash',1,$(ConvertTo-RapIntakeSql $now)) ON CONFLICT(StoreId) DO UPDATE SET Payload=excluded.Payload,RecordHash=excluded.RecordHash,SchemaVersion=excluded.SchemaVersion,UpdatedUtc=excluded.UpdatedUtc;COMMIT;";[Rap.NativeSqlite]::Execute($path,$sql,5000);$Store
}
function Assert-RapIntakePersistedBody($Body){
    if($null-eq$Body-or[long]$Body.SchemaVersion-ne1){throw 'RESEARCH_INTAKE_SCHEMA_UNSUPPORTED'}
    foreach($n in @('KnownProjectIds','Questions','Searches','Inbox','Promotions','IdentityResults','LookupResults','Decisions','Lifecycles','Audit','Counters','ProductionWrite','ProductionPilot')){if(-not$Body.PSObject.Properties[$n]){throw "RESEARCH_INTAKE_PERSISTENCE_MISSING:$n"}}
    ...
    foreach($p in $Body.Promotions.PSObject.Properties){$x=$p.Value;if($p.Name-cne$x.CandidateId-or-not$Body.Inbox.PSObject.Properties[$p.Name]-or$x.State-cne'PROMOTED'){throw 'RESEARCH_INTAKE_PERSISTENCE_CORRUPT'}}
...
function Open-RapResearchIntakeStore {
    ...
    try{$json=[Text.Encoding]::UTF8.GetString([Convert]::FromBase64String([string]$payload));$body=$json|ConvertFrom-Json -Depth 80}catch{throw 'RESEARCH_INTAKE_PERSISTENCE_CORRUPT'};if((Get-RapIntakePersistenceHash $json)-cne$columnHash){throw 'RESEARCH_INTAKE_PERSISTENCE_CORRUPT'};if([long]$schema-ne1){throw 'RESEARCH_INTAKE_SCHEMA_UNSUPPORTED'};Assert-RapIntakePersistedBody $body
    $audit=[Collections.Generic.List[object]]::new();foreach($e in @($body.Audit)){$audit.Add($e)}
    [pscustomobject]@{PSTypeName='Rap.ResearchIntakeStore';SchemaVersion=1;DatabasePath=[IO.Path]::GetFullPath($DatabasePath);KnownProjectIds=@($body.KnownProjectIds);Questions=ConvertFrom-RapIntakeMap $body.Questions 'Rap.ResearchQuestion';Searches=ConvertFrom-RapIntakeMap $body.Searches '';Inbox=ConvertFrom-RapIntakeMap $body.Inbox 'Rap.ResearchInboxCandidate';Promotions=ConvertFrom-RapIntakeMap $body.Promotions 'Rap.ResearchPromotion';IdentityResults=ConvertFrom-RapIntakeMap $body.IdentityResults 'Rap.IdentityResolution';LookupResults=ConvertFrom-RapIntakeMap $body.LookupResults 'Rap.IntakeLookupResult';Decisions=ConvertFrom-RapIntakeMap $body.Decisions 'Rap.ZoteroDecision';Lifecycles=ConvertFrom-RapIntakeMap $body.Lifecycles '';CanonicalPapers=@($body.CanonicalPapers);ZoteroItems=@($body.ZoteroItems);Audit=$audit;Counters=$body.Counters;ProductionWrite='DISABLED';ProductionPilot='TEST_DEFERRED'}
}
