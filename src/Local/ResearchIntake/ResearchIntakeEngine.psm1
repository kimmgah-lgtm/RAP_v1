...
function Resolve-RapCanonicalPaperIdentity {
    ...
    $persisted=Assert-RapPromotionBinding $Store $Promotion;$candidate=$Store.Inbox[$Promotion.CandidateId];$Store.Lifecycles[$candidate.CandidateId]='IDENTITY_RESOLVING';if($Store.DatabasePath){[void](Save-RapResearchIntakeStore $Store)}
...
function Get-RapPreZoteroDedupDecision {
    ...
    [void](Assert-RapPromotionBinding $Store $Promotion);if($Promotion.CandidateId-cne$IdentityResolution.CandidateId){throw 'CANDIDATE_SUBSTITUTION'}
    ...
    elseif($canonicalStatus-ceq'NOT_FOUND'-and$zoteroStatus-ceq'NOT_FOUND'){$decisionName='CREATE_CANDIDATE';$reason='AUTHORITATIVE_NOT_FOUND';$zoteroId=$null;$Store.Counters.ZoteroCreateDecision++}
    else{$decisionName='BLOCKED';$reason='LOOKUP_STATE_NOT_CREATE_AUTHORITY';$zoteroId=$null}
    $decision=[pscustomobject]@{PSTypeName='Rap.ZoteroDecision';ProjectId=$Promotion.ProjectId;ResearchQuestionId=$Promotion.ResearchQuestionId;CandidateId=$Promotion.CandidateId;PromotionHash=$Promotion.PromotionHash;Decision=$decisionName;Reason=$reason;CanonicalKey=$IdentityResolution.CanonicalKey;CanonicalStatus=$canonicalStatus;ZoteroStatus=$zoteroStatus;LookupExecutionId=$snapshot.ExecutionId;LookupExecutedAt=$snapshot.ExecutedAt;CanonicalMatches=$canonicalMatches.Count;ZoteroMatches=$zoteroMatches.Count;ZoteroItemId=$zoteroId;State=$(if($decisionName-in@('CREATE_CANDIDATE','REUSE_EXISTING')){$decisionName}else{$decisionName});ProductionWrite='DISABLED';ChangesApplied=$false}
    $Store.Decisions[$Promotion.CandidateId]=$decision;$Store.Lifecycles[$Promotion.CandidateId]=$decision.State;[void](Add-RapIntakeAudit $Store 'PRE_ZOTERO_DEDUP_DECIDED' $decision.State @{CandidateId=$Promotion.CandidateId;CanonicalKey=$decision.CanonicalKey;CanonicalStatus=$decision.CanonicalStatus;ZoteroStatus=$decision.ZoteroStatus;LookupExecutionId=$decision.LookupExecutionId;LookupExecutedAt=$decision.LookupExecutedAt;CanonicalMatches=$decision.CanonicalMatches;ZoteroMatches=$decision.ZoteroMatches;Decision=$decision.Decision;Reason=$decision.Reason});$decision
}
