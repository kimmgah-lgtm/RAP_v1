...
function Add-RapIntakeAudit { param($Store,[string]$Event,[string]$State,[hashtable]$Data) $entry=[pscustomobject][ordered]@{Sequence=$Store.Audit.Count+1;Event=$Event;State=$State;Timestamp=[DateTimeOffset]::UtcNow.ToString('o');Data=[pscustomobject]$Data;ProductionWrite='DISABLED'};$Store.Audit.Add($entry);$entry }
...
function New-RapResearchIntakeStore {
    [CmdletBinding()]param([Parameter(Mandatory)][string[]]$KnownProjectIds,[object[]]$ExistingCanonicalPapers=@(),[object[]]$ExistingZoteroItems=@())
    foreach($project in $KnownProjectIds){Assert-RapProjectId $project}
    [pscustomobject]@{PSTypeName='Rap.ResearchIntakeStore';KnownProjectIds=@($KnownProjectIds|Sort-Object -Unique);Questions=@{};Searches=@{};Inbox=@{};Promotions=@{};Decisions=@{};CanonicalPapers=@($ExistingCanonicalPapers|ForEach-Object{Copy-RapIntakeValue $_});ZoteroItems=@($ExistingZoteroItems|ForEach-Object{Copy-RapIntakeValue $_});Audit=[Collections.Generic.List[object]]::new();Counters=[pscustomobject]@{ZoteroCreateDecision=0;ZoteroMutation=0;DriveMutation=0;NotionMutation=0;LibraryIdAllocation=0;PaperReviewCreation=0};ProductionWrite='DISABLED';ProductionPilot='TEST_DEFERRED'}
}
...
function Add-RapResearchInboxCandidates {
    ...
    $added=@();foreach($result in $SearchResults){if($result.PSObject.TypeNames-cnotcontains'Rap.SearchResult'){throw 'ENTITY_BOUNDARY_VIOLATION'};if([string]::IsNullOrWhiteSpace([string]$result.Provenance)){throw 'MISSING_PROVENANCE'};if($Store.Inbox.ContainsKey($result.CandidateId)){throw 'DUPLICATE_CANDIDATE_ID'}
        ...
        $Store.Inbox[$candidate.CandidateId]=$candidate;$added+=Copy-RapIntakeValue $candidate;[void](Add-RapIntakeAudit $Store 'CANDIDATE_INBOXED' 'INBOXED' @{ProjectId=$candidate.ProjectId;ResearchQuestionId=$candidate.ResearchQuestionId;SearchExecutionId=$candidate.SearchExecutionId;CandidateId=$candidate.CandidateId;Reason=$Reason})
    };@($added)
}
...
function Promote-RapResearchInboxCandidate {
    ...
    $Store.Promotions[$CandidateId]=$promotion;$candidate.State='PROMOTED';[void](Add-RapIntakeAudit $Store 'CANDIDATE_PROMOTED' 'PROMOTED' @{ProjectId=$ProjectId;ResearchQuestionId=$ResearchQuestionId;CandidateId=$CandidateId;Researcher=$Researcher;PromotionHash=$promotion.PromotionHash;Provenance=$Provenance});$promotion
}
...
function Resolve-RapCanonicalPaperIdentity {
    ...
    [void](Add-RapIntakeAudit $Store 'IDENTITY_RESOLVED' $state @{CandidateId=$candidate.CandidateId;Status=$status;Authority=$authority;CanonicalKey=$key;ObservedIdentifiers="doi=$doi;pmid=$pmid;pmcid=$pmcid;publisher=$publisher"});$resolution
}
...
function Get-RapPreZoteroDedupDecision {
    ...
    $candidate=$Store.Inbox[$Promotion.CandidateId];$canonicalMatches=@($Store.CanonicalPapers|Where-Object{[string]$_.CanonicalKey-ceq$IdentityResolution.CanonicalKey});$zoteroMatches=@($Store.ZoteroItems|Where-Object{[string]$_.CanonicalKey-ceq$IdentityResolution.CanonicalKey})
    $conflicts=@($canonicalMatches+$zoteroMatches|Where-Object{Test-RapStrongMetadataConflict $candidate $_})
    if($conflicts.Count-gt0){$decisionName='IDENTITY_CONFLICT';$reason='IDENTIFIER_METADATA_CONFLICT';$zoteroId=$null}
    ...
    $Store.Decisions[$Promotion.CandidateId]=$decision;[void](Add-RapIntakeAudit $Store 'PRE_ZOTERO_DEDUP_DECIDED' $decision.State @{CandidateId=$Promotion.CandidateId;CanonicalKey=$decision.CanonicalKey;CanonicalMatches=$decision.CanonicalMatches;ZoteroMatches=$decision.ZoteroMatches;Decision=$decision.Decision;Reason=$decision.Reason});$decision
}
