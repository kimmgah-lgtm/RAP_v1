...
$dataset=New-RapAnalysisDataset -Specification $spec -Records $records
...
Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($ai)} 'RESEARCHER_CONFIRMATION_REQUIRED' 'TEST 02 AI input admitted.'
$wrong=New-RapSynthesisRecord 'LIB:L000004' ES-X .2 -ProjectId PR002
Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($wrong)} 'PROJECT_SCOPE_MISMATCH' 'TEST 03 project boundary failed.'
...
$depSpec=New-RapAnalysisSpecification -AnalysisId AN-DEP -ProjectId PR001 -DependencyStrategy RVE
$depData=New-RapAnalysisDataset $depSpec $dependent
...
Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($missing)} 'ANALYSIS_INPUT_REQUIRED' 'TEST 06 missing input did not fail.'
$badN=New-RapSynthesisRecord 'LIB:L000011' ES-N .2 -SampleSize -1
Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($badN)} 'INVALID_SAMPLE_SIZE' 'TEST 07 invalid N did not fail.'
$badV=New-RapSynthesisRecord 'LIB:L000012' ES-V .2 -Variance 0
Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($badV)} 'INVALID_VARIANCE' 'TEST 08 invalid variance did not fail.'
$unsupported=New-RapSynthesisRecord 'LIB:L000013' ES-U .2;$unsupported.EffectSizeType='Odds_Ratio'
Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($unsupported)} 'UNSUPPORTED_EFFECT_CONVERSION' 'TEST 09 unsupported conversion did not fail.'
$conflict=New-RapSynthesisRecord 'LIB:L000014' ES-C .2 -EvidenceStatus CONFLICTING_EVIDENCE
Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($conflict)} 'UNRESOLVED_CONFLICT' 'TEST 10 conflict admitted.'
...
$derivedData=New-RapAnalysisDataset $spec @($derivedRecord)
...
Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($needs)} 'DIRECTION_HARMONIZATION_REQUIRED' 'TEST 17 silent reversal occurred.'
$reverse=New-RapSynthesisRecord 'LIB:L000016' ES-R .4 -DirectionAction REVERSE -DirectionConfirmed $true
$reverseData=New-RapAnalysisDataset $spec @($reverse)
...
# 20-23 dataset
$datasetAgain=New-RapAnalysisDataset $spec @($records|Sort-Object EffectSizeId -Descending)
...
$changed=New-RapBasicSynthesisFixture;$changed[0].EffectValue=.11;$changedData=New-RapAnalysisDataset $spec $changed
...
try{
  ...
  $noDep=New-RapAnalysisSpecification -AnalysisId AN-NODEP -ProjectId PR001
  Assert-RapSynthThrows {New-RapAnalysisDataset $noDep $dependent} 'DEPENDENCY_STRATEGY_REQUIRED' 'TEST 38 silent independence assumed.'
  Assert-RapSynth ($depData.DependencyStrategy -eq 'RVE') 'TEST 39 dependency strategy missing.'
  $modSpec=New-RapAnalysisSpecification -AnalysisId AN-MOD -ProjectId PR001 -ModeratorSelection Region
  $modData=New-RapAnalysisDataset $modSpec $records;$sub=Invoke-RapSubgroupAnalysis $modSpec $modData Region
  ...
  $sensitivity=Invoke-RapSensitivityAnalysis $spec $dataset
  ...
  Assert-RapSynth (@($sensitivity.Runs|Select-Object ExcludedEffectSizeId -Unique).Count -eq 3) 'TEST 45 exclusions not auditable.'
  ...
  # 49-52 graph lineage
  Assert-RapSynth (@($result.Run.Lineage|Where-Object Source -like 'EFFECT_SIZE:*').Count -eq 3) 'TEST 49 result-effect lineage missing.'
  Assert-RapSynth ($result.Run.Lineage[0].Evidence -like 'EVIDENCE-*') 'TEST 50 effect-evidence trace missing.'
  ...
  Assert-RapSynth ($null -ne $derivedResult.Run.Lineage[0].StatisticalInputs) 'TEST 51 derived-input trace missing.'
  ...
  Assert-RapSynth ($after.Runs[0].Lineage[0].Evidence -like 'EVIDENCE-*') 'TEST 57 provenance reload failed.'
  ...
  Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($human)} 'HUMAN_OWNED_FIELD_BLOCKED' 'TEST 60 human field writable.'
  $blank=New-RapSynthesisRecord 'LIB:L000021' ES-BLANK .2;$blank|Add-Member ReviewerMemo ''
  Assert-RapSynthThrows {New-RapAnalysisDataset $spec @($blank)} 'HUMAN_OWNED_FIELD_BLOCKED' 'TEST 61 blank human field writable.'
