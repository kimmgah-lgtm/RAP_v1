...
A ((Get-RapPrThrown {New-RapExternalReadAdapter Zotero ([uri]'http://example.com/') RAP_TEST_CREDENTIAL {}})-match'HTTPS') 'remote HTTP rejected'
...
$unknown=New-RapExternalReadAdapter Notion ([uri]'https://api.notion.com/v1/') RAP_TEST_CREDENTIAL {param($r)[pscustomobject]@{Authorized=$true}};A ((Invoke-RapExternalReadProbe $unknown).Status-ne'PASS') 'partial external state fails closed'
