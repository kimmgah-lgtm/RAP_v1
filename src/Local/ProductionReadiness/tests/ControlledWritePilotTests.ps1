...
$adapter=New-RapControlledWriteFixtureAdapter Notion 'page-L000003' @{RAP_Status='draft'} 7
$plan=P CW01;$approval=ApproveTestPlan $plan;$result=Invoke-RapControlledWritePilot $plan $approval $adapter
$state=Get-RapControlledWriteFixtureState $adapter
A CW01 ($result.Status-eq'VERIFIED'-and$result.Verification-eq'PASS'-and$state.Fields.RAP_Status-eq'reviewed'-and$state.MutationCount-eq1-and$result.Audit.ReadBackVerified-and($result.StateHistory-join'>')-eq'PLANNED>AWAITING_APPROVAL>APPROVED>APPLYING>APPLIED>VERIFIED') 'approved single mutation is read back, verified, and audited'

$adapter=New-RapControlledWriteFixtureAdapter Notion 'page-L000003' @{RAP_Status='draft'} 7;$plan=P CW02;$request=New-RapControlledWriteApprovalRequest $plan
$noApproval=(E {Invoke-RapControlledWritePilot $plan $null $adapter})-match'HUMAN_APPROVAL_REQUIRED'
...
A CW02 ($noApproval-and$selfApproval-and(Get-RapControlledWriteFixtureState $adapter).MutationCount-eq0) 'missing or automated self-approval blocks apply'
...
$adapter=New-RapControlledWriteFixtureAdapter Notion 'page-L000003' @{RAP_Status='changed-after-plan'} 7;$plan=P CW06;$approval=ApproveTestPlan $plan
...
$adapter=New-RapControlledWriteFixtureAdapter Notion 'page-L000003' @{RAP_Status='draft'} 7;$plan=P CW07;$approval=ApproveTestPlan $plan;$plan.ProposedValue='tampered'
...
$adapter=New-RapControlledWriteFixtureAdapter Notion 'page-L000003' @{RAP_Status='draft'} 7;$plan=P CW08;$approval=ApproveTestPlan $plan;$null=Invoke-RapControlledWritePilot $plan $approval $adapter;$again=Invoke-RapControlledWritePilot $plan $approval $adapter
...
$plan2=P CW08 'draft' 'different';$approval2=ApproveTestPlan $plan2
A CW09 ((E {Invoke-RapControlledWritePilot $plan2 $approval2 $adapter})-match'OPERATION_REPLAY_CONFLICT'-and(Get-RapControlledWriteFixtureState $adapter).MutationCount-eq1) 'reused Operation_ID with new payload is blocked'
