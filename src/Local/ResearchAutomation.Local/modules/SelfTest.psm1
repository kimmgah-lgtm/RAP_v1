...
function Invoke-RapSelfTest {
    ...
    $checks.Add((New-RapCheck 'Synthesis Engine' $synthesisSafe 'SPR-009 Final Gate PASS; lineage-validated local/mock synthesis loaded; production writes disabled'))
    $outputCommands=@('New-RapOutputSpecification','New-RapOutputDataset','New-RapResearchArtifact','New-RapReproducibilityManifest','New-RapExportPackage','Invoke-RapOutputGeneration','Save-RapOutputPackage')
    ...
    $checks.Add((New-RapCheck 'Workflow Exceptions' $workflowSafe 'SPR-011 complete; exception policy loaded; production writes disabled'))
    $reconciliationCommands=@('Compare-RapIntegritySnapshot','New-RapReconciliationDecision','New-RapRecoveryPlan','Invoke-RapRecoveryPlan','Test-RapRecoveryReadBack','New-RapReconciliationSqliteDependencies')
    $reconciliationLoaded=@($reconciliationCommands|Where-Object{Get-Command $_ -CommandType Function -ErrorAction SilentlyContinue}).Count -eq $reconciliationCommands.Count
    $reconciliationSafe=$reconciliationLoaded -and $Configuration.capabilities.Reconciliation -eq $true -and $Configuration.capabilities.ReconciliationProductionWrite -eq $false
    $checks.Add((New-RapCheck 'Reconciliation Engine' $reconciliationSafe 'SPR-011.5 local/fixture recovery loaded; production writes disabled'))
    $readinessCommands=@('New-RapEnvironmentGuard','Invoke-RapExternalReadProbe','Invoke-RapProductionWriteFirewall','New-RapMutationManifest','Test-RapProductionPreflight','Invoke-RapProductionDryRun')
    $readinessLoaded=@($readinessCommands|Where-Object{Get-Command $_ -CommandType Function -ErrorAction SilentlyContinue}).Count-eq$readinessCommands.Count
    $readinessSafe=$readinessLoaded-and$Configuration.capabilities.ProductionReadiness-eq$true-and$Configuration.capabilities.ProductionReadinessProductionWrite-eq$false-and$Configuration.productionReadiness.productionWriteEnabled-eq$false
    $checks.Add((New-RapCheck 'Production Readiness' $readinessSafe 'SPR-012 read-only probes and preflight loaded; production writes disabled'))
